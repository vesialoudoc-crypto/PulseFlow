...
Redis uses StackExchange.Redis connect and async timeouts, with `WaitAsync` only for
caller cancellation. RabbitMQ's fixed publisher slots discard uncertain channels,
remain recoverable for later batches without retrying the earlier batch, and make
readiness unhealthy if none are usable. `Startup` owns one linked budget for the
complete startup sequence, and `HealthChecks` sets built-in readiness-registration
