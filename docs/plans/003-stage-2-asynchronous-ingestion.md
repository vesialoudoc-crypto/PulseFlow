...
- broker-side flow control and RabbitMQ prefetch. The adapter invokes the application
  delivery handler directly and does not add an application-level queue or buffer. A
  reliability step must decide the acknowledgement outcome when parsing or persistence
  fails before selecting prefetch;
